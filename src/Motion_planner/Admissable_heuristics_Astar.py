...
def plot(L,R,theta,A):
    ...
    if(len(X)>0):
        ...
        for j in range(len(count)):
            ...
            plt.plot(X[count[j][0]:count[j][0]+count[j][1]],Y[count[j][0]:count[j][0]+count[j][1]],'r')
...
if __name__=="__main__":
    ...
    goal = node(0, 12.5, 12.5, 0, None, None)
