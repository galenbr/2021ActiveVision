...
ros::Publisher concat_array_pub;
...
std_msgs::Float32MultiArray concat_array;
/////////////////////////////
...
float * highest_point (const PointCloud::Ptr temp, pcl::PointXYZ minimum, pcl::PointXYZ maximum,int which_case )
{
  ...
  float unexplored_vector [25] = {};
  int j = 0;
  for(int i = 0; i<6; i++)
  {
    std::cout<<"X vector:"<<X[i]<<std::endl;
    std::cout<<"Y vector:"<<Y[i]<<std::endl;
  }
    std::cout<<"Zmin vector:"<<minimum.z<<std::endl;
    std::cout<<"Zmax vector:"<<maximum.z<<std::endl;

  if( which_case == 1)
  {
    concat_array.data.clear();
  }
  for (int x = 0; x < 5; x++)
  {
    for (int y= 0; y<5; y++)
    {
      ...
      for (int i = 0; i< temp->points.size(); ++i)
      {
        ...
        {
          if(temp->points[i].z > highest_z && which_case == 1 )
            { highest_z = temp->points[i].z;
              explored_vector[j] = highest_z;}
          if(temp->points[i].z > highest_z && which_case == 2 )
            { highest_z = temp->points[i].z;
              unexplored_vector[j] = highest_z;}
        }
      }
      j++;
    }
  ...
  {
    for(int i = 0;i<25;i++)
      {concat_array.data.push_back(explored_vector[i]);
       std::cout<<"Explored vector is: "<<explored_vector[i]<<std::endl;} 
  }

if(which_case==2)
{
  for(int i = 0;i<25;i++)
  {concat_array.data.push_back(unexplored_vector[i]);
       std::cout<<"Unxplored vector is: "<<unexplored_vector[i]<<std::endl;} 
  for(int i = 0; i<50;i++)
  {
    std::cout<<"concat array: "<<concat_array.data[i]<<std::endl;
  }
  concat_array_pub.publish(concat_array);  
}
...
//updatedPCL = newSegPC;//testing
  ...
  *exploredpcd_worldframe = *source + *target;
  ...
  explored_haf_pcd = highest_point(exploredpcd_worldframe,minPt,maxPt,1);
  std::cout<<"explored pcd world frame size is:  "<< exploredpcd_worldframe->size()<<std::endl;
  pcl::io::savePCDFileASCII ("Explored_test.pcd", *exploredpcd_worldframe);
  ...
  pcl::getMinMax3D (*unexplored_initial, minPt, maxPt);
  float *explored_haf_pcd;
  explored_haf_pcd = highest_point(unexplored_initial,minPt,maxPt,2);
  pcl::io::savePCDFileASCII ("Unxplored_test.pcd", *unexplored_initial);
  ...
  // unexplored_direction_updated = unexplored_direction;
  pcl::getMinMax3D (*unexplored_direction, minPt, maxPt);
  float *explored_haf_pcd;
  explored_haf_pcd = highest_point(unexplored_direction,minPt,maxPt,2);
  }
  ...
  pcl::getMinMax3D (*unexplored_direction_updated, minPt, maxPt);
  float *explored_haf_pcd;
  explored_haf_pcd = highest_point(unexplored_direction_updated,minPt,maxPt,2);
  }
  ...
  {
    storedPCL_new_direction = updatedPCL;
    std::cout<<"Stored a point cloud new direction" <<std::endl;
  }
  if(index_2 == 1)
  {	
...
int main (int argc, char** argv)
{
  ...
  concat_array_pub = nhptr->advertise<std_msgs::Float32MultiArray>("concat_array_topic",1000);
