...
X_vertices = np.array([w0/2, w0, w0/2, -w0/2, -w0, -w0])
Y_vertices = np.array([float(w0*sqrt(3)/2), 0, -float(w0*sqrt(3)/2), -float(w0*sqrt(3)/2), 0, float(w0*sqrt(3)/2)])
...
class visual_servoing:
	...
	def anticlockwise(self):
		...
		R = np.array([[cos(self.Block_orientation), -sin(self.Block_orientation), self.x], [sin(self.Block_orientation), cos(self.Block_orientation), self.y], [0, 0, 1]])
		coords = np.dot(R, np.concatenate(([X_vertices], [Y_vertices], np.ones((1,n)))))
		...
		alpha = atan2(coords[1][contact_right-1] - coords[1][contact_right], coords[0][contact_right-1] - coords[0][contact_right])
		print contact_right
		print alpha
		...
		self.rotate_object_anticlockwise(theta_ref)

		while(1):
			theta_ref = theta_ref - 0.02
			self.rotate_object_anticlockwise(theta_ref)
			time.sleep(1)
			...
			if(abs(alpha - encoder_gripper_angle_conversion(theta_ref, 1)) < 10):
				break
	...
	def clockwise(self):
		...
		theta_ref = theta[0]
		theta_ref = theta_ref - 0.05
		self.rotate_object_clockwise(theta_ref)
		...
		while(1):
			...
			alpha = atan2(coords[1][contact_left] - coords[1][contact_left+1], coords[0][contact_left] - coords[0][contact_left+1])

			if(abs(-alpha*180./np.pi + encoder_gripper_angle_conversion(theta_ref, 0)) < 7):
				break
