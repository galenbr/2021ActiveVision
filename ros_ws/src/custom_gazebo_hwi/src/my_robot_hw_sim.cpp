  ...
  std::vector<transmission_interface::TransmissionInfo> transmissions)
{
  ...
  for(unsigned int j=0; j < n_dof_; j++)
  {
    ...
    {
      ROS_WARN_STREAM_NAMED("my_robot_hw_sim","Transmission " << transmissions[j].name_
        ...
        !(transmissions[j].actuators_[0].hardware_interfaces_.empty()))
    {
      ...
      ROS_WARN_STREAM_NAMED("my_robot_hw_sim", "The <hardware_interface> element of tranmission " <<
    ...
    if (joint_interfaces.empty())
    {
      ROS_WARN_STREAM_NAMED("my_robot_hw_sim", "Joint " << transmissions[j].joints_[0].name_ <<
    ...
    // Debug
    ROS_DEBUG_STREAM_NAMED("my_robot_hw_sim","Loading joint '" << joint_names_[j]
    ...
    {
      ROS_FATAL_STREAM_NAMED("my_robot_hw_sim","No matching hardware interface found for '"
    ...
    if (!joint)
    {
      ROS_ERROR_STREAM_NAMED("my_robot_hw", "This robot has a joint named \"" << joint_names_[j]
    ...
    {
      ROS_WARN_STREAM_NAMED("my_robot_hw_sim", "No physics type found.");
    }
      ...
      case POSITION_PID:
        {
          ...
          sim_joints_[j]->SetForce(0, effort);
        //   ROS_INFO_STREAM("**effort_for_j1**"<<joint_position_command_[0]);
        }
        break;
...
void MyRobotHWSim::eStopActive(const bool active)
{
...
void MyRobotHWSim::registerJointLimits(const std::string& joint_name,
...
PLUGINLIB_EXPORT_CLASS(custom_gazebo_hwi::MyRobotHWSim, gazebo_ros_control::RobotHWSim)
